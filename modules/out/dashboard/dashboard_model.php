...
class Dashboard
{
    ...
    public function create($apikey,$ds_name)
    {
       
        $this->mysqli->query("INSERT INTO tbl_dashboard (`apikey`,`ds_name`) VALUES ('$apikey','$ds_name')");
    ...
    {
        $id = (int) $id;
        $result = $this->mysqli->query("DELETE FROM tbl_dashboard WHERE id = '$id'");
        return $result;
    }
    ...
    public function get_list($apikey)
    {
        $result = $this->mysqli->query("SELECT id, ds_name, description FROM tbl_dashboard WHERE apikey='$apikey'");
    ...
    public function set_content($apikey, $ds_name, $content, $height)
    {
        $height = (int) $height;
        ...
        $result = $this->mysqli->query("SELECT * FROM tbl_dashboard WHERE `apikey` = '$apikey' AND `ds_name`='$ds_name'");
        $row = $result->fetch_array();
        if ($row) $this->mysqli->query("UPDATE tbl_dashboard SET `content` = '$content', `height` = '$height' WHERE `apikey` = '$apikey' AND `ds_name` ='$ds_name''");

        return array('success'=>true);
    }

    public function set($apikey,$ds_name,$fields)
    {
        ...
        $this->mysqli->query("UPDATE tbl_dashboard SET ".$fieldstr." WHERE `apikey`='$apikey' and `ds_name` = '$ds_name'");
    ...
    */
    public function get($ds_name)
    {
      
        $result = $this->mysqli->query("SELECT * FROM tbl_dashboard WHERE  `ds_name`='$ds_name'");
     
        if ($result->num_rows != 1)  { err_sql($this->mysqli->error,$_SERVER['PHP_SELF'],'No dashboard with this ds_name:'.$ds_name); }
