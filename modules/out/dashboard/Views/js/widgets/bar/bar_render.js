...
function bar_widgetlist()
{
	...
	addOption(widgets["bar"], "templ_var",        "templ_var",              "templ_var",                "templ_var value",                                                                  []);
	addOption(widgets["bar"], "max",         "value",             "Max value",           "Max value to show",                                                           []);
	addOption(widgets["bar"], "scale",       "value",             "Scale",               "Value is multiplied by scale before display. Defaults to 1",                  []);
	addOption(widgets["bar"], "units",       "value",             "Units",               "Unit type to show after value. Ex: <br>\"{Reading}{unit-string}\"",           []);
	addOption(widgets["bar"], "offset",      "value",             "Offset",              "Static offset. Subtracted from value before computing position (default 0)",  []);
	addOption(widgets["bar"], "colour",      "colour_picker",     "Colour",              "Colour to draw bar in",                                                       []);
	addOption(widgets["bar"], "graduations", "dropbox",           "Graduations",         "Should the graduations be shown",                                             graduationDropBoxOptions);
	addOption(widgets["bar"], "gradNumber",  "value",             "Num Graduations",     "How many graduation lines to draw (only relevant if graduations are on)",     []);
...
function bar_draw()
{
	...
	{
		var templ_var = $(this).attr("templ_var");
		var val = curve_value(templ_var,dialrate);
		// ONLY UPDATE ON CHANGE
		if ((val * 1).toFixed(2) != (assoc[templ_var] * 1).toFixed(2) || redraw == 1)
		{
