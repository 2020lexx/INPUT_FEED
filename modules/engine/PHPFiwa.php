...
*   Logger Controller
*   
*   Customized by: Pablo E. Miguel
*
*/
// This timeseries engine implements:  this is the main core db data process ---
...
class PHPFiwa
{
    ...
    */
     ...
     * Create feed
     *
     * @param integer $logger_log_file_id The id of the feed to be created
    */
    public function create($logger_log_file_id,$options)
    {
        ...
        if (!$meta = $this->get_meta($logger_log_file_id))
        {
            deb_log("PHPFIWA:create  feed logger_log_file_id=$logger_log_file_id",0);
            // Set initial feed meta data
            $meta = new stdClass();
            $meta->logger_log_file_id = $logger_log_file_id;
            $meta->start_time = 0;
            ...
            // Save meta data
            $this->create_meta($logger_log_file_id,$meta);
            
            $fh = fopen($this->dir.$meta->logger_log_file_id."_0.dat", 'c+');
            fclose($fh);
            $fh = fopen($this->dir.$meta->logger_log_file_id."_1.dat", 'c+');
            fclose($fh);
            $fh = fopen($this->dir.$meta->logger_log_file_id."_2.dat", 'c+');
            fclose($fh);
            $fh = fopen($this->dir.$meta->logger_log_file_id."_3.dat", 'c+');
            fclose($fh);
        }

        $feedname = "$logger_log_file_id.meta";
          deb_log("PHPFIWA:got logger file meta:".$feedname,0);
     ...
     *
     * @param char The logger_log_file_id of the feed to add to
     * @param integer $time The unix timestamp of the data point, in seconds
     * @param float $value The value of the data point
    */
    public function post($logger_log_file_id,$timestamp,$value)
    {   
        deb_log("PHPFiwa:post logger_log_file_id=$logger_log_file_id timestamp=$timestamp value=$value",0);

       //++ $logger_log_file_id = (int) $id;
        ...
        if ($timestamp<$start || $timestamp>$end) {
            deb_log("PHPFiwa:post timestamp out of range",0);
            return false;
        }
        ...
        if (!$meta = $this->get_meta($logger_log_file_id)) {
            deb_log("PHPFiwa:post failed to fetch meta logger_log_file_id=$logger_log_file_id",0);
            return false;
        }
        ...
        if ($timestamp < $meta->start_time) {
            deb_log("PHPFiwa:post timestamp older than feed start time logger_log_file_id=$logger_log_file_id",0);
        ...
        if ($point<=$last_point) {
             // $this->log->warn("PHPFiwa:post updating of datapoints to be made via update function logger_log_file_id=$logger_log_file_id");
    ...
    {
        $fh = fopen($this->dir.$meta->logger_log_file_id."_$layer.dat", 'c+');
        if (!$fh) {
            deb_log("PHPFiwa:update_layer could not open data file layer $layer logger_log_file_id=".$meta->logger_log_file_id,0);
            return false;
        }
        
        if (!flock($fh, LOCK_EX)) {
           deb_log("PHPFiwa:update_layer data file for layer=$layer feedlogger_log_file_id=".$meta->logger_log_file_id." is locked by another process",0);
            fclose($fh);
            return false;
        }
        ...
        if ($padding>0) {
            if ($this->write_padding($fh,$meta->npoints[$layer],$padding)===false)
            {
               ...
               deb_log("PHPFiwa:update_layer padding max block size exeeded $padding logger_log_file_id=".$meta->logger_log_file_id,0);
                return false;
            }
        }
     ...
     *
     * @param integer $logger_log_file_id The id of the feed to add to
     * @param integer $time The unix timestamp of the data point, in seconds
     * @param float $value The value of the data point
    */
    public function update($logger_log_file_id,$time,$value)
    {
     ...
     *
     * @param integer $logger_log_file_id The id of the feed to fetch from
     * @param integer $start The unix timestamp in ms of the start of the data range
     * @param integer $end The unix timestamp in ms of the end of the data range
     * @param integer $dp The number of data points to return (used by some engines)
    */
    public function get_data_basic($logger_log_file_id,$start,$end,$dp)
    {
       // $logger_log_file_id = intval($logger_log_file_id);
        ...
        // If meta data file does not exist then exit
        if (!$meta = $this->get_meta($logger_log_file_id)) return false;
    ...
    public function get_data($logger_log_file_id,$start,$end,$outinterval)
    {
      //  $logger_log_file_id = intval($logger_log_file_id);
        $start = intval($start/1000);
        $end = intval($end/1000);
        ...
        $fh = fopen($this->dir.$meta->logger_log_file_id."_$layer.dat", 'rb');
        fseek($fh,$startpos*4);
     ...
     *
     * @param integer $logger_log_logger_log_file_id The id of the feed
    */
    public function lastvalue($logger_log_file_id)
    {
       // $logger_log_file_id = (int) $logger_log_file_id;
        
        // If meta data file does not exist then exit
        if (!$meta = $this->get_meta($logger_log_file_id)) return false;
        if ($meta->npoints[0]>0)
        {
            $fh = fopen($this->dir.$meta->logger_log_file_id."_0.dat", 'rb');
    ...
    public function export($logger_log_file_id,$start,$layer)
    {
       // $logger_log_file_id = (int) $logger_log_file_id;
        $start = (int) $start;
        $layer = (int) $layer;
        
        $feedname = $logger_log_file_id."_$layer.dat";
    ...
    public function delete($logger_log_file_id)
    {
        if (!$meta = $this->get_meta($logger_log_file_id)) return false;
        unlink($this->dir.$meta->logger_log_file_id.".meta");
        for ($i=0; $i<$meta->nlayers; $i++)
        {
          unlink($this->dir.$meta->logger_log_file_id."_$i.dat");
        }
    }
    
    public function get_feed_size($logger_log_file_id)
    {
        if (!$meta = $this->get_meta($logger_log_file_id)) return false;
        
        $size = 0;
        $size += filesize($this->dir.$meta->logger_log_file_id.".meta");
        $size += filesize($this->dir.$meta->logger_log_file_id."_0.dat");
        $size += filesize($this->dir.$meta->logger_log_file_id."_1.dat");
        $size += filesize($this->dir.$meta->logger_log_file_id."_2.dat");
        $size += filesize($this->dir.$meta->logger_log_file_id."_3.dat");
        return $size;
    }
    

    public function get_meta($logger_log_file_id)
    {
      //  $logger_log_file_id = (int) $logger_log_file_id;
        $feedname = "$logger_log_file_id.meta";
        ...
        if ($meta->nlayers<1 || $meta->nlayers>4) {
            deb_log("PHPFiwa:get_meta feed:$id nlayers out of range",0);
            return false;
        }
        ...
        for ($i=0; $i<$meta->nlayers; $i++)
        {
          clearstatcache($this->dir.$meta->logger_log_file_id."_$i.dat");
          $meta->npoints[$i] = floor(filesize($this->dir.$meta->logger_log_file_id."_$i.dat") / 4.0);
        }
        
        
        if ($meta->start_time <= 0 && $meta->npoints[0]>1) {
            deb_log("PHPFiwa:get_meta feed:$logger_log_file_id start time must be greater than zero",0);
        }
        
        if ($meta->start_time>0 && $meta->npoints[0]==0) {
           deb_log("PHPFiwa:get_meta start_time already defined but npoints is 0",0);
        }
        
        return $meta;
    }

    public function create_meta($logger_log_file_id,$meta)
    {
        //$logger_log_file_id = (int) $logger_log_file_id;
        $feedname = "$logger_log_file_id.meta";
    ...
    public function recompile($meta)
    {
        ...
        $fh = fopen($this->dir.$meta->logger_log_file_id."_$layer.dat", 'c+');
        $fh1 = fopen($this->dir.$meta->logger_log_file_id."_1.dat", 'c+');
        $fh2 = fopen($this->dir.$meta->logger_log_file_id."_2.dat", 'c+');    
        $fh3 = fopen($this->dir.$meta->logger_log_file_id."_3.dat", 'c+');
    ...
    public function csv_export($logger_log_file_id,$start,$end,$outinterval)
    {
        ...
        header('Content-Description: File Transfer');
        header("Content-type: application/octet-stream");
        $filename = $logger_log_file_id.".csv";
